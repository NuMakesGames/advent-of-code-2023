	...
	void PrintSolution(const std::filesystem::path& inputFile, bool shouldRender)
	{
		...
		for (int i = 0; i < galaxyPositions.size() - 1; ++i)
		{
			...
			for (int j = i + 1; j < galaxyPositions.size(); ++j)
			{
				...
				// Distance of the universe expansion is the sum of the empty rows and columns between the galaxies
				int64_t expansionDistance = rowsBetweenPositions + columnsBetweenPositions;
