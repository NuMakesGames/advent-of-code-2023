...
IMPLEMENT_STD_HASH(Puzzle16B::Ray, value.direction, value.start);
...
namespace Puzzle16B
{
	...
	int64_t CountEnergizedTiles(const Grid2d<char>& grid, const std::unordered_map<Ray, Vector2d<int>>& reflectedRays, const Ray& sourceRay)
	{
		// Copy the reflected rays and add the source ray
		std::unordered_map<Ray, Vector2d<int>> rays = reflectedRays;
		...
		return std::ranges::count(energizedTiles, '#');
	}
	...
	void PrintSolution(const std::filesystem::path& inputFile, bool shouldRender)
	{
		...
		// Try all source rays
		int64_t maxEnergizedTiles = 0;

		for (int x = 0; x < grid.Width(); ++x)
		{
			maxEnergizedTiles = std::max(maxEnergizedTiles, CountEnergizedTiles(grid, rays, { down, { x, -1 } }));
			maxEnergizedTiles = std::max(maxEnergizedTiles, CountEnergizedTiles(grid, rays, { up, { x, grid.Height() } }));
		}

		for (int y = 0; y < grid.Height(); ++y)
		{
			maxEnergizedTiles = std::max(maxEnergizedTiles, CountEnergizedTiles(grid, rays, { right, { -1, y } }));
			maxEnergizedTiles = std::max(maxEnergizedTiles, CountEnergizedTiles(grid, rays, { left, { grid.Width(), y } }));
		}
